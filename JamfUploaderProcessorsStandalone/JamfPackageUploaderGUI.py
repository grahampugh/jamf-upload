...
"""
JamfPackageUploaderGUI processor for AutoPkg
    by G Pugh
...
class JamfPackageUploaderGUI(Processor):
    ...
    Note: This is an experimental processor that utilises the GUI method of updating
    packages. Use JamfPackageUploader for production use."""
    ...
    def scrape_upload_token(self, session, jamf_url):
        ...
        url = "{}/legacy/packages.html".format(jamf_url)  # TEST
        r = session.get(url)
        text = r.text

        self.output("huge amount of html follows", verbose_level=3)
        self.output("------", verbose_level=3)
        self.output(text, verbose_level=3)
        self.output("------", verbose_level=3)

        # matches = re.search(r'data-base-url="([^"]*)"', text)
        matches = re.search(r'const url = "([^"]*)"', text)
        if matches is None:
            self.output("WARNING: No JCDS distribution point URL was found")
            self.output("- Are you sure that JCDS is your Primary distribution point?")

        jcds_base_url_urlencoded = matches.group(1)

        matches = re.search(r'data-upload-token="([^"]*)"', text)
        if matches is None:
            self.output("WARNING: No JCDS upload token was found")
            self.output("- Are you sure that JCDS is your Primary distribution point?")
        ...
        if matches is None:
            self.output("WARNING: No package upload session token was found")
        else:
            self.output("Package upload session token was found")
    ...
    def post_pkg_chunks(
        ...
        for chunk in range(0, total_chunks):
            ...
            self.output("URL to post chunks: {}".format(chunk_url), verbose_level=2)

            r = http.post(chunk_url, files={"file": chunk_reader}, headers=headers)
            self.output("Uploaded chunk {} of {}".format(chunk + 1, total_chunks))
            self.output(r, verbose_level=3)

        resource.close()

    def update_pkg_by_form(
        self, session, session_token, jamf_url, pkg_name, pkg_path, obj_id, pkg_metadata
    ):
        ...
        self.output(r.content, verbose_level=2)

        if r.status_code == 200:
            self.output("Successfully created package")
            query = urlparse(r.url).query
            matches = re.search(r"id=([^&]*)", query)
            if matches is None:
                self.output("No package id in redirected url")
            else:
                pkg_id = matches.group(1)
                self.output("Package ID: {}".format(pkg_id))
                return pkg_id
        else:
    ...
    def main(self):
        ...
        r, login_session = self.login(self.jamf_url, self.jamf_user, self.jamf_password)
        if r.status_code != 200:
            self.output(
                "Failed to log in to the Jamf instance at: {}".format(self.jamf_url)
            )
        ...
        # otherwise process for cloud DP
            ...
            jcds_url, jcds_token, session_token = self.scrape_upload_token(
                login_session, self.jamf_url
            )
            if jcds_url and jcds_token and session_token:
                self.output("JCDS URL: {}".format(jcds_url), verbose_level=1)
                self.output("JCDS Upload token: {}".format(jcds_token), verbose_level=1)
                self.output("Session token: {}".format(session_token), verbose_level=1)

                chunksize = 1

                # post the package as chunks
                self.post_pkg_chunks(
                    self.pkg_name,
                    self.pkg_path,
                    jcds_url,
                    jcds_token,
                ...
                pkg_id = self.update_pkg_by_form(
                    login_session,
                    session_token,
                    self.jamf_url,
                    self.pkg_name,
