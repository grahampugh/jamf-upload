...
class JamfPackageCleaner(JamfUploaderBase):
    ...
    def main(self):
        ...
        if len(self.pkg_name_match) < self.minimum_name_length:
            self.output(
                f"'pkg_name_match' argument ({self.pkg_name_match}) needs at least {self.minimum_name_length} characters. Override by changing the 'minimum_name_length' argument. Aborting."
            )
            return
        ...
        if len(found_packages) <= self.versions_to_keep:
            self.output(
                f"No need to delete any packages. Only {len(found_packages)} found. Keeping up to {self.versions_to_keep} versions"
            )
        ...
        if len(packages_to_delete) > self.maximum_allowed_packages_to_delete:
            self.output(
                f"Too many matches. Found {len(packages_to_delete)} to delete. Maximum allowed is {self.maximum_allowed_packages_to_delete}. Override by setting the 'maximum_allowed_packages_to_delete' argument. Aborting."
            )
            return
