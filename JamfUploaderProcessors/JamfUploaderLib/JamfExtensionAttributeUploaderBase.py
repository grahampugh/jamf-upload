...
class JamfExtensionAttributeUploaderBase(JamfUploaderBase):
    ...
    def upload_ea(
        ...
        if os.path.exists(script_path):
            with open(file=script_path, mode="r", encoding="utf-8") as file:
    ...
    def execute(self):
        ...
        replace = self.env.get("replace_ea")
        ...
        if obj_id:
            ...
            if replace:
                self.output(
                    f"Replacing existing Extension Attribute as 'replace_ea' is set to {replace}",
                    verbose_level=1,
                )
            else:
                self.output(
