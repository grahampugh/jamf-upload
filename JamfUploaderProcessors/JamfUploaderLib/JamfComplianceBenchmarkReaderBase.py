...
class JamfComplianceBenchmarkReaderBase(JamfUploaderBase):
    ...
    def execute(self):
        ...
        platform_url = self.env.get("PLATFORM_URL").rstrip("/")
        client_id = self.env.get("PLATFORM_CLIENT_ID")
        client_secret = self.env.get("PLATFORM_CLIENT_SECRET")
        ...
        if "jamfcompliancebenchmarkreader_summary_result" in self.env:
            del self.env["jamfcompliancebenchmarkreader_summary_result"]
        ...
        if all_objects or list_only:
            self.output(f"Getting all {object_type} objects from {platform_url}")
            object_list = self.get_all_api_objects(
                platform_url, object_type, uuid="", token=token, namekey=namekey
            )
            if list_only:
                ...
                if output_dir:
                    ...
                    output_filename = (
                        f"{client_id}-{self.object_list_types(object_type)}.json"
                    )
                    file_path = os.path.join(output_dir, output_filename)
                    try:
                        with open(file_path, "w", encoding="utf-8") as fp:
        ...
        # output the summary
        self.env["jamfcompliancebenchmarkreader_summary_result"] = {
