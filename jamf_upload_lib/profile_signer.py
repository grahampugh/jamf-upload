...
"""
Taken from https://github.com/nmcspadden/ProfileSigner/blob/master/profile_signer.py
Requires a python3 environment with Foundation installed
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Sign or encrypt mobileconfig profiles, using either a cert + key file, or a keychain certificate."
    )
    parser.add_argument(
        "sign",
    ...
    parser.add_argument(
        "outfile",
        help="Path to output .mobileconfig file. Defaults to outputting into the same directory.",
    )
    ...
    if args.sign == "encrypt" or args.sign == "both":
        ...
        proc = subprocess.Popen(
            security_cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE
        )
        (sout, serr) = proc.communicate()
        if serr:
            print("Error: %s" % serr, file=sys.stderr)
            sys.exit(1)
        ...
        try:
            ...
            print("Could not write to file!", file=sys.stderr)
            sys.exit(1)
        ...
        if encerr:
            print("Error: %s" % encerr, file=sys.stderr)
            sys.exit(1)
        ...
        wrapped_data = NSData.dataWithBytes_length_(
            binaryEncPayload, len(binaryEncPayload)
        )
    ...
    if args.sign == "sign" or args.sign == "both":
        ...
        if not args.name:
            print(
                "Error: A certificate common name is required to sign profiles with the Keychain.",
                file=sys.stderr,
            )
            sys.exit(22)
        if args.sign == "both":
