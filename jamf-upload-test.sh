...
while test $# -gt 0 ; do
    case "$1" in
        ...
        --replace) 
            ...
            elif [[ $processor == "JamfSoftwareRestrictionUploaderTest" ]]; then
        ...
        --process_name|--process-name) 
            shift
            if [[ $processor == "JamfSoftwareRestrictionUploaderTest" ]]; then
...
if [[ $verbosity -le 1 ]]; then
    ...
    /Library/AutoPkg/Python3/Python.framework/Versions/Current/bin/python3 "$DIR/JamfUploaderTest/$processor.py" < "$temp_processor_plist" > "$temp_receipt"
    echo
    echo "Output:"
    ...
    /Library/AutoPkg/Python3/Python.framework/Versions/Current/bin/python3 "$DIR/JamfUploaderTest/$processor.py" < "$temp_processor_plist"
fi
